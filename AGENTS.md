...
- **폰트** — 정적 경로라 그대로 선언. 본문 폰트(Pretendard Regular/Medium)는 전 경로(`/*`), 순번 숫자 폰트(Outfit-ExtraLight)는 홈(`/`)만. `@font-face` 요청은 익명 CORS 모드라 `crossorigin` 필수(없으면 preload 파일 재사용 못 하고 중복 다운로드).
